...
CREATE TABLE `rentals` (
  `rental_id` TINYINT PRIMARY KEY NOT NULL AUTO_INCREMENT,
  `inventory_id` MEDIUMINT NOT NULL AUTO_INCREMENT,
  `customer_id` SMALLINT NOT NULL AUTO_INCREMENT,
  `rental_status_id` TINYINT NOT NULL AUTO_INCREMENT,
  `staff_id` TINYINT NOT NULL AUTO_INCREMENT
);

CREATE TABLE `rental_status` (
  `rental_status_id` TINYINT PRIMARY KEY NOT NULL AUTO_INCREMENT
);
...
CREATE INDEX `idx_fk_inventory_id` ON `rentals` (`inventory_id`);

CREATE INDEX `idx_fk_customer_id` ON `rentals` (`customer_id`);

CREATE INDEX `idx_fk_rental_status_id` ON `rentals` (`rental_status_id`);

CREATE INDEX `idx_fk_staff_id` ON `rentals` (`staff_id`);
...
ALTER TABLE `category` ADD FOREIGN KEY (`category_id`) REFERENCES `film` (`category_id`) ON DELETE RESTRICT ON UPDATE CASCADE;
...
ALTER TABLE `inventory` ADD FOREIGN KEY (`inventory_id`) REFERENCES `rentals` (`inventory_id`);

ALTER TABLE `customer` ADD FOREIGN KEY (`customer_id`) REFERENCES `rentals` (`customer_id`);

ALTER TABLE `rental_status` ADD FOREIGN KEY (`rental_status_id`) REFERENCES `rentals` (`rental_status_id`);

ALTER TABLE `staff` ADD FOREIGN KEY (`staff_id`) REFERENCES `rentals` (`staff_id`);
